...
import org.evosuite.ga.metaheuristics.mosa.ReadabilityMOSA;
...
public class PropertiesSuiteGAFactory extends PropertiesSearchAlgorithmFactory<TestSuiteChromosome> {
	...
	protected GeneticAlgorithm<TestSuiteChromosome> getGeneticAlgorithm(ChromosomeFactory<TestSuiteChromosome> factory) {
		...
		case RMOSA:
			logger.info("Chosen search algorithm: RMOSA");
			return new ReadabilityMOSA<>(factory);
		case QMOSA:
			logger.info("Chosen search algorithm: QMOSA");
			return new QualityMOSA<>(factory);
		default:
