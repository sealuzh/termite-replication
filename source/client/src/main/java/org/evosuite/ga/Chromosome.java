...
public abstract class Chromosome implements Comparable<Chromosome>, Serializable,
	...
	private double coupling = Double.NaN;
	private double archiveCoupling = Double.NaN;

	private boolean isFromSlicing = false;
...
//				 || ( value < 0 ) || ( ff == null )) 
	...
	/**
	 * Returns the quality index as defined by Palomba et.al. with the ISSTA 2016 paper
	...
	public void setQuality(double cohesion, double coupling) {
		setCoupling(coupling);
		setCohesion(cohesion);
		this.quality = 0.5 * cohesion + 0.5 * coupling;
	}
	...
	public int isFromSlicing() {
		return isFromSlicing ? 1 : 0;
	}

	public void setFromSlicing(boolean fromSlicing) {
		isFromSlicing = fromSlicing;
	}

	public double getArchiveCoupling() {
		return archiveCoupling;
	}

	public void setArchiveCoupling(double archiveCoupling) {
		this.archiveCoupling = archiveCoupling;
	}
}
