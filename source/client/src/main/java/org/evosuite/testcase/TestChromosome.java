...
import org.evosuite.ga.metaheuristics.mosa.Slice;
...
public class TestChromosome extends ExecutableChromosome {
	...
	/** Maps the occurrences of the term for this test case
	 * Used for the textual analysis*/
	private TermsOccurrences termsOccurrences;

	/**
	 * Computes the map for the occurrences of the terms in this test case and fill the
	 * {@link TermsOccurrences} instance variable
	 */
	public void computeOccurrencesMap() {
		termsOccurrences = new TermsOccurrences("testChromosome", this.getTestCase().toCode());
	}

	/**
	 * Returns the {@link TermsOccurrences} object for the current chromosome;
	 * It contains the map of the terms occurrences
	 */
	public TermsOccurrences getTermsOccurrences() { return termsOccurrences; }
	...
	public Chromosome clone() {
		...
		c.setFromSlicing(isFromSlicing()==1);
	...
	*/
	...
	/**
	 * Creates a new chromosome from the slice
	 * @param slice a {@link Slice} object
	 * @return the newly constucted chromosome
	 * @throws ConstructionFailedException
	 */
	public TestChromosome getTestChromosomeFromSlice(Slice slice) throws ConstructionFailedException {

		TestChromosome tc = new TestChromosome();
		TestFactory testFactory = TestFactory.getInstance();

		for (Statement statement : slice.getStatementsOfSlice()) {
			testFactory.appendStatement(tc.getTestCase(), statement);
		}
		tc.setChanged(true);
		return tc;
	}
	...
	public int getNumberOfVariableReferences() {
		Set<VariableReference> variableReferences = new HashSet<>();

		for (Statement statement : this.getTestCase()) {
			variableReferences.addAll(statement.getVariableReferences());
		}
	...
	private void computeEntropy() {
		...
		ExecutionTrace trace = getLastExecutionResult().getTrace();

		for (TestFitnessFunction testFitnessFunction : this.getTestCase().getCoveredGoals()) {
			if (testFitnessFunction instanceof BranchCoverageTestFitness) {
				String methodSignature = testFitnessFunction.getTargetClass() + "."
						+ testFitnessFunction.getTargetMethod();
				coverages.put(methodSignature, coverages.getOrDefault(methodSignature, 0) + 1);
			}
		}

		// For each covered branchless method (not in coverages), we put the number of covered branches at 1.
		for (String coveredMethod : trace.getCoveredBranchlessMethods())
				coverages.put(coveredMethod, 1);

		double totalCoverage = coverages.values().stream().reduce(Integer::sum).orElse(0);
		assert totalCoverage >= 0 : "Coverage is negative!";

		double entropy = 0D;
		for (Integer methodCoverage : coverages.values()) {
			double percentCoverage = (methodCoverage / totalCoverage);

			double oldEntropy = entropy;
			entropy += percentCoverage * Math.log(percentCoverage);

			assert oldEntropy >= entropy : "Entropy has decreased!";
		}

		double normalizedSwitches = getContextSwitches(trace);
		this.entropy = -entropy + normalizedSwitches;
		// set the value of the computed entropy also in the TC
		// todo: improve this at some point
		this.test.setCoverageEntropy(-entropy + normalizedSwitches);
	}

	/**
	 * Computes and normalizes the context switches; we need CBRANCH or IBRANCH as a criterion
	 * for the computation (or a flag that regulates the context analysis).
	 * Look at the @{@link ExecutionTraceImpl} class for more details
	 * @param trace
	 * @return the number of context switches normalized between 0 and 1
	 */
	private double getContextSwitches(ExecutionTrace trace) {
		Map<String, Map<CallContext, Integer>> methodContextCount = trace.getMethodContextCount();
		double sum = 0D;
		Collection<Map<CallContext, Integer>> values = methodContextCount.values();
		for (Map<CallContext, Integer> entry: values)
			sum += entry.values().stream().reduce(0, Integer::sum);
		sum = sum/(sum+1);
		if (Double.isNaN(sum))
			return 0D;
		return sum/(sum+1);
