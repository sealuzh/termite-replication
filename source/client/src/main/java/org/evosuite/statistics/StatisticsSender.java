...
public class StatisticsSender {
	...
	public static void executedAndThenSendIndividualToMaster(TestSuiteChromosome testSuite) throws IllegalArgumentException{
		...
		/** sends the coverage entropy info */
		sendCoverageEntropyInfo(testSuite);
		sendSlicingInfo(testSuite);
		sendCohesionAndQualityInfo(testSuite);
		sendExceedingAmountInfo(testSuite);
	}

	// -------- private methods ------------------------

	private static void sendExceedingAmountInfo(TestSuiteChromosome testSuite) {
		String sourcePath = Properties.SOURCE_PATH;
		SmellinessDetector detector = new SmellinessDetector(sourcePath);
		int threshold = 1;
		double sum = 0;
		for (TestChromosome chromosome: testSuite.getTestChromosomes()) {
			List<TermsOccurrences> coveredOccurrences = detector.getCoveredOccurrences(chromosome.getTestCase().toCode());
			logger.debug("Covered Methods =  " + coveredOccurrences.size());
			if (coveredOccurrences.size()==0)
				sum+=0;
			else
				sum+=coveredOccurrences.size()-threshold;
		}
		ClientServices.getInstance().getClientNode().trackOutputVariable(
				RuntimeVariable.EagerTestExceedingAmounts, sum/(double)testSuite.getTestChromosomes().size());
	}

	/**
	 * Sends the information about the average cohesion and coupling for the resulting
	 * final test suite
	 * @param testSuite the final test suite
	 */
	private static void sendCohesionAndQualityInfo(TestSuiteChromosome testSuite) {
		String sourcePath = Properties.SOURCE_PATH;
		SmellinessDetector detector = new SmellinessDetector(sourcePath);
		...
		Set<TermsOccurrences> occurrences = new HashSet<>();
		/** add the occurrences to a set */
		for (TestChromosome chromosome: chromosomes) {
			TermsOccurrences termsOccurrences = chromosome.getTermsOccurrences();
			if (termsOccurrences == null) {
				chromosome.computeOccurrencesMap();
				termsOccurrences = chromosome.getTermsOccurrences();
			}
			occurrences.add(termsOccurrences);
		}

		for (TestChromosome chromosome: chromosomes) {
			cohesion += detector.evaluateCohesion(chromosome.getTestCase().toCode());
			TermsOccurrences current = chromosome.getTermsOccurrences();
			occurrences.remove(current);
			coupling += detector.evaluateCoupling(current, new ArrayList<>(occurrences));
			occurrences.add(current);
		}

		ClientServices.getInstance().getClientNode().trackOutputVariable(
				RuntimeVariable.LackOfCohesion, cohesion/(double)populationSize);
		ClientServices.getInstance().getClientNode().trackOutputVariable(
				RuntimeVariable.Coupling, coupling/(double)populationSize);
	}
	...
	/**
	 * Sends the information about how many tests originally generated from slicing ends up in the final solutions
	 * @param testSuite the final test suite
	 */
	private static void sendSlicingInfo(TestSuiteChromosome testSuite) {
		List<TestChromosome> chromosomes = testSuite.getTestChromosomes();
		int tcFromSlicing = chromosomes.stream().mapToInt(d -> d.isFromSlicing()).sum();
		logger.debug("TC from slice {} over {}", tcFromSlicing, chromosomes.size());
		ClientServices.getInstance().getClientNode().trackOutputVariable(
				RuntimeVariable.TestsFromSlicing, tcFromSlicing
		);
	}

	/**
	 * Report the information about the coverage entropy for the generated test suite
	 * @param testSuite the final test suite
	 */
	private static void sendCoverageEntropyInfo(TestSuiteChromosome testSuite) {
		List<TestChromosome> chromosomes = testSuite.getTestChromosomes();
		double testSuiteEntropy = chromosomes.stream().mapToDouble(d -> d.getEntropy()).average().orElse(Double.NaN);

		ClientServices.getInstance().getClientNode().trackOutputVariable(
				RuntimeVariable.CoverageEntropy, testSuiteEntropy);
	}
