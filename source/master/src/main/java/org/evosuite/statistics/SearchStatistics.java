...
public class SearchStatistics implements Listener<ClientStateInformation>{
	...
	private SearchStatistics() { 
		...
		/** Quality metrics */
		sequenceOutputVariableFactories.put(RuntimeVariable.CoverageEntropyTimeline.name(), new CoverageEntropySequenceOutputVariableFactory());
		sequenceOutputVariableFactories.put(RuntimeVariable.LackOfCohesionTimeline.name(), new LackOfCohesionSequenceOutputVariableFactory());
		sequenceOutputVariableFactories.put(RuntimeVariable.CouplingTimeline.name(), new CouplingSequenceOutputVariableFactory());
...
//            v.setValue((Integer)variable.getValue());
	...
	/**
	 * Track Coverage Entropy over time
	 */
	private static class CoverageEntropySequenceOutputVariableFactory extends SequenceOutputVariableFactory<Double> {
		public CoverageEntropySequenceOutputVariableFactory() {
			super(RuntimeVariable.CoverageEntropyTimeline);
		}
		@Override
		protected Double getValue(TestSuiteChromosome individual) {
			List<TestCase> tests = individual.getTests();
			if (tests.isEmpty())
				return 0D;
			double overallEntropy = tests.stream().map(t -> t.getCoverageEntropy()).reduce(Double::sum).get();
			return overallEntropy/(double)tests.size();
		}
	}

	/**
	 * Track LackOfCohesion over time
	 */
	private static class LackOfCohesionSequenceOutputVariableFactory extends SequenceOutputVariableFactory<Double> {
		public LackOfCohesionSequenceOutputVariableFactory() {
			super(RuntimeVariable.LackOfCohesionTimeline);
		}
		@Override
		protected Double getValue(TestSuiteChromosome individual) {
			...
			if (tests.isEmpty())
				return 0D;
			double overallCohesion = tests.stream().map(t -> t.getCohesion()).reduce(Double::sum).get();
			return overallCohesion/(double)tests.size();
		}
	}
	 ...
	 */
	private static class CouplingSequenceOutputVariableFactory extends SequenceOutputVariableFactory<Double> {
		public CouplingSequenceOutputVariableFactory() {
			super(RuntimeVariable.CouplingTimeline);
		}
		@Override
		protected Double getValue(TestSuiteChromosome individual) {
			List<TestChromosome> tests = individual.getTestChromosomes();
			if (tests.isEmpty())
				return 0D;
			double overallCohesion = tests.stream().map(t -> t.getCoupling()).reduce(Double::sum).get();
			return overallCohesion/(double)tests.size();
		}
	}
