...
def getEvoSuiteCall(seed, configId, config, project, clazz, cpath, max_iteration ,id, strategy, coreIndex):
  global SCRIPTDIR
  ...
  # everthing is in the dir under /lib goes into the classopath
  class_path = []
  for root, dirnames, filenames in os.walk(CASESTUDY_DIR+''+project):
    for filename in fnmatch.filter(filenames, '*.jar'):
        class_path.append(os.path.join(root, filename))
  ...
  result += ' -Dentropy_comments=true'
  result += ' -Dconfiguration_id=' + CONFIG_NAME
  result += " -projectCP " + class_path
  result += ' -Doutput_variables=TARGET_CLASS,criterion,configuration_id,Total_Goals,Total_Branches,Lines,Covered_Goals,Generations,Statements_Executed,Fitness_Evaluations,Tests_Executed,Generations,Total_Time,Size,Result_Size,Length,Result_Length,Random_Seed,BranchCoverage,LineCoverage,OutputCoverage,ExceptionCoverage,WeakMutationScore,MethodCoverage,CoverageEntropy,MutationScore,LackOfCohesion,Coupling,EagerTestExceedingAmounts'
  result += " -Dgroup_id="+project

  if CONFIG_NAME != 'WHOLE-SUITE':    
    result += " -Dalgorithm=" + ALGORITHM

  if ALGORITHM == 'RMOSA':
    result += FLAG

  result += " -DsourcePath="+SOURCE_DIR + cpath
  
  result += " -Dstopping_condition=MAXGENERATIONS"
  result += " -Dsearch_budget="+max_iteration
  result += " "+config+" "+FIXED
...
def createJobs(minSeed, maxSeed, configId, config, startNum, strategy="-generateSuite"):
  ...
  for seed in range(minSeed, maxSeed):
    ...
    for entry in CLASSES:
      ...
      script.write(getEvoSuiteCall(seed, configId, config, entry[0], entry[1], entry[2], entry[3], JOB_ID, strategy, coreIndex))
...
createJobs(MINSEED, MAXSEED, CONFIG_NAME , " " , 0, STRATEGY)
